...
class loginController extends Controller
{
    ...
    public function refresh(Request $request){
        ...
        try{
            ...
            $isSecure=config('session.secure');
            $sub=config('session.accepted_sub');
            return response()->json(['message'=>'Invalid or expired token'], 401)->withCookie(cookie('network', $sub, -1, '/', null, $isSecure, true, false, $same_site));
        }
    }
    ...
    public function logout(Request $request){
        ...
        $isSecure=config('session.secure');
        $sub=config('session.accepted_sub');
        return response()->json(['status'=>200, 'message'=>'Logged Out'])->withCookie(cookie('network', $sub, -1, '/', null, $isSecure, true, false, $same_site));
